...
void menuPrincipal(){
	...
	bool band = true; //Crea una se�al para deterner el do-while
...
void menuEdicion(){
	...
	bool band1 = true; //Crea una se�al para deterner el do-while
...
void abrirjson(string** punteromatriz, int f, int c){
	ifstream jsonFile("archivo.json");
	json root;
  	jsonFile >> root;

  if (root["matriz"].is_array()) {
    int x=0;
    for (const auto& fila : root["matriz"]) {
      int y=0;
      if (fila.is_array()) {
        for (const auto& elemento : fila) {
          *(*(punteromatriz+x)+y) = elemento;
          y+=1;
        }
      }
		x+=1;
...
void guardarMatriz(string **punteromatriz, int f, int c) {
    ...
    for (int i = 0; i < f; i++) {
        ...
        // Verifica si no es la última fila
        if (i != f - 1) {
